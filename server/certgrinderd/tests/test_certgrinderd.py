...
def test_get_ocsp_response_no_aia(certificate_file_no_aia: Path, caplog: pytest.LogCaptureFixture) -> None:
    """Test the get_ocsp_response() method with a cert with no AIA."""
    certgrinderd = Certgrinderd({"log-level": "DEBUG", "preferred-chain": "Fake LE Root X2"})
    with pytest.raises(SystemExit) as e:
        certgrinderd.get_ocsp_response(certpath=certificate_file_no_aia)
    assert e.type is SystemExit, f"Exit was not as expected, it was {e.type}"
    assert e.value.code == 1, "Exit code not 1 as expected"
    assert "No AUTHORITY_INFORMATION_ACCESS extension found in the certificate" in caplog.text, (
...
def test_get_ocsp_response_not_http_200(
    certificate_chain_file: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
...
def test_check_ocsp_response_not_successful(
    signed_certificate: cryptography.x509.Certificate, caplog: pytest.LogCaptureFixture
) -> None:
    """Test the check_ocsp_response() method with an unsuccessful OCSP response."""
    certgrinderd = Certgrinderd({"log-level": "DEBUG"})
    assert (
        certgrinderd.check_ocsp_response(
            ocsp_request=certgrinderd.create_ocsp_request(certificate=signed_certificate, issuer=signed_certificate),
    ...
    assert "OCSP response status is not SUCCESSFUL, it is OCSPResponseStatus.UNAUTHORIZED" in caplog.text


def test_check_ocsp_response_wrong_serial(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    known_private_key: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    ...
    ).responder_id(cryptography.x509.ocsp.OCSPResponderEncoding.HASH, signed_certificate)
    ocsp_response = builder.sign(known_private_key, cryptography.hazmat.primitives.hashes.SHA256())
    ...
    assert "The OCSP response has a different serial_number than the OCSP request" in caplog.text
...
def test_check_ocsp_response_wrong_hash_algo(
    ...
    assert "The OCSP response has a different hash_algorithm than the OCSP request" in caplog.text
...
def test_check_ocsp_response_wrong_issuer_key(
    ...
    assert "The OCSP response has a different issuer_key_hash than the OCSP request" in caplog.text


def test_check_ocsp_response_this_update_in_future(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    known_private_key_2: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    builder = builder.add_response(
        ...
        algorithm=cryptography.hazmat.primitives.hashes.SHA1(),
        cert_status=cryptography.x509.ocsp.OCSPCertStatus.GOOD,
        this_update=datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(days=1),
        next_update=datetime.datetime.now(tz=datetime.timezone.utc),
        revocation_time=None,
        revocation_reason=None,
    ).responder_id(cryptography.x509.ocsp.OCSPResponderEncoding.HASH, selfsigned_certificate)
    ocsp_response = builder.sign(known_private_key_2, cryptography.hazmat.primitives.hashes.SHA256())
    ...
    assert "The this_update parameter of the OCSP response is in the future" in caplog.text
...
def test_check_ocsp_response_no_next_update(
    ...
    assert "OCSP response has no nextUpdate attribute. This violates RFC5019 2.2.4." in caplog.text
...
def test_check_ocsp_response_next_update_in_past(
    ...
    builder = builder.add_response(
        ...
        cert_status=cryptography.x509.ocsp.OCSPCertStatus.GOOD,
        this_update=datetime.datetime.now(tz=datetime.timezone.utc),
        next_update=datetime.datetime.now(tz=datetime.timezone.utc) - datetime.timedelta(days=1),
        revocation_time=None,
        revocation_reason=None,
    ).responder_id(cryptography.x509.ocsp.OCSPResponderEncoding.HASH, selfsigned_certificate)
    ocsp_response = builder.sign(known_private_key_2, cryptography.hazmat.primitives.hashes.SHA256())
    ...
    assert "The next_update parameter of the OCSP response is in the past" in caplog.text


def test_check_ocsp_response_delegated_signer(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    delegated_signer_certificate: cryptography.x509.Certificate,
    known_private_key_3: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    ocsp_response = builder.sign(known_private_key_3, cryptography.hazmat.primitives.hashes.SHA256())
    assert (
        certgrinderd.check_ocsp_response(
            ocsp_request=certgrinderd.create_ocsp_request(
                certificate=signed_certificate, issuer=selfsigned_certificate
    ...
    assert "Found OCSP responder cert with the right namehash and keyhash in OCSP response:" in caplog.text
...
def test_check_ocsp_response_delegated_signer_not_signed_by_issuer(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    delegated_signer_certificate_not_signed_by_issuer: cryptography.x509.Certificate,
    known_private_key_3: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    assert "The OCSP responder certificate is not signed by certificate issuer" in caplog.text


def mock_verify_signature_fail(
    pubkey: cryptography.hazmat.primitives.asymmetric.rsa.RSAPublicKey,
    signature: bytes,
    payload: bytes,
...
def test_check_ocsp_response_delegated_signer_no_extendedkeyusage(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    delegated_signer_certificate_no_eku: cryptography.x509.Certificate,
    known_private_key_3: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    assert "No ExtendedKeyUsage extension found in delegated OCSP responder certificate" in caplog.text


def test_check_ocsp_response_delegated_signer_no_ocsp_perm(
    signed_certificate: cryptography.x509.Certificate,
    selfsigned_certificate: cryptography.x509.Certificate,
    delegated_signer_certificate_no_ocsp_perm: cryptography.x509.Certificate,
    known_private_key_3: cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey,
    caplog: pytest.LogCaptureFixture,
) -> None:
    ...
    assert "Delegated OCSP responder certificate is not permitted to sign OCSP responses" in caplog.text
